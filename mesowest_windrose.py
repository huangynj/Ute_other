...
# A wrapper for the windrose script I've downloaded from online and copied into pyBKB
# Requries some modules in the pyBKB library for downloading MesoWest data and plotting a windrose
...
def rose_with_labels():    
    ax = new_axes()
    ax.bar(wd, ws, bins = [0,1,2,3,4,5,6,7,8],nsector = 16, \
                   normed=True, \
                   opening=.95, edgecolor='k')
    set_legend(ax)
    plt.title("Wind Rose "+a['station name']+"\n"+start.strftime('%H:%M UTC %d%b%Y')+" - "+end.strftime('%H:%M UTC %d%b%Y')+ "\n")
    ...
    plt.savefig("wind_hi.png",bbox_inches="tight",dpi=500)
    #plt.show()
def rose_no_labels():    
    """
    ...
    #ax.set_rmax(25)
    ax.set_rmax(np.max(np.sum(ax._info['table'],axis=0)))
    plt.savefig("wind_hi_nolabel.png",bbox_inches="tight",dpi=500, transparent=True)
    #plt.show()
...
def clock_rose():
    ...
    ax.contourf(hour, ws,  bins = [0,1,2,3,4,5,6,7,8], nsector = 24,\
                    normed=True)
                   
    set_legend(ax)
    plt.title("Wind Clock "+a['station name']+"\n"+start.strftime('%H:%M UTC %d%b%Y')+" - "+end.strftime('%H:%M UTC %d%b%Y')+ "\n")
