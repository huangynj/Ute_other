...
# Requires the ozonerose script for plotting windroses found in pyBKB for plotting a windrose
# As well as the BB_MesoWest for getting data from the MesoWest API
...
from ozonerose import WindroseAxes
...
def rose_with_labels():    
    ...
    plt.savefig("hi.png",bbox_inches="tight",dpi=500)
    #plt.show()
def rose_no_labels():    
    """
    ...
    #ax.set_rmax(25)
    ax.set_rmax(np.max(np.sum(ax._info['table'],axis=0)))
    plt.savefig("hi_nolabel.png",bbox_inches="tight",dpi=500, transparent=True)
    #plt.show()
...
def clock_rose():
    ...
    set_legend(ax)
    plt.title("Ozone Clock "+a['station name']+"\n"+start.strftime('%H:%M UTC %d%b%Y')+" - "+end.strftime('%H:%M UTC %d%b%Y')+ "\n")
